...
beforeEach()
    ->fixtures('installer-extra-directories')
    ->prepare();

afterEach()
    ->cleanup();

test('install-path points directly to the extra directory', function () {
    ...
    expect($package['install-path'])->toBe('../../libraries/dex/fake');
});
